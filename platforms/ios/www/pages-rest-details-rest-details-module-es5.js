            ...
            this.api.post('stores/getByUid', body).then(function (datas) {
              ...
              if (datas && datas.status === 200 && datas.data.length > 0) {
                ...
                if (data) {
                  ...
                  _this2.open = moment__WEBPACK_IMPORTED_MODULE_7__('10-10-2020 ' + data.open_time).format('LT');
                  _this2.close = moment__WEBPACK_IMPORTED_MODULE_7__('10-10-2020 ' + data.close_time).format('LT');
                  _this2.phone = data.mobile;
                  var format = 'HH:mm:ss';
                  var currentTime = moment__WEBPACK_IMPORTED_MODULE_7__().format(format); // console.log(currentTime);

                  var time = moment__WEBPACK_IMPORTED_MODULE_7__(currentTime, format);
                  var beforeTime = moment__WEBPACK_IMPORTED_MODULE_7__(data.open_time, format);
                  var afterTime = moment__WEBPACK_IMPORTED_MODULE_7__(data.close_time, format);

                  if (time.isBetween(beforeTime, afterTime)) {
                    // console.log('is between');
                    _this2.isOpen = true;
                  } else {
                    _this2.isOpen = false; // console.log('is not between');
                  }
                }
              }
            }, function (error) {
